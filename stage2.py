...
def compute_heart_rates():
    ...
    if method == 'heart_sound_method':
        ...
        # heart_rate_filter = signal.butter(2, 0.999, 'lowpass', output='sos')
        # heart_rates = signal.sosfilt(heart_rate_filter, compute_heart_rate(peaks_radar, fs_radar))
        # heart_rates = arma_est_bip_mm.arma_est_bip_mm(compute_heart_rate(peaks_radar, fs_radar), 1, 0)['cleaned_signal']
        heart_rates = running_robust_location(compute_heart_rate(peaks_radar, fs_radar), method='m_loc_hub', n=3, c=0.01)
